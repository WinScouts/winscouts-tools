 ...
 if (debug) { do_debug(i+" "+ role_detail.data[i].unitid+" "+role_detail.data[i].UnitName+" "+role_detail.data[i].teamname+" "+role_detail.data[i].TeamId+" "+role_detail.data[i].Role); }
         ...
         //      var w = welcome.data.find(e=>e.MembershipNumber == role_detail.data[i].membershipid && e.CategoryKey == act[j]);
              ...
              if(w==null){
               ...
               if (w!=null) {
                   if (debug) { do_debug("Welcome Status for "+ role_detail.data[i].FullName+"/"+ role_detail.data[i].membershipid+"/"+act[j]+"/Role Status"+ role_detail.data[i].RoleStatusName);}
                 if (debug) { do_debug("Status "+w.Status+"/"+w.Role+"/"+w['Category key']+"/"+w['Membership number']+w['Unit name']);}
                   ...
                   if (role_detail.data[i].RoleStatusName!="Full") {
                       ...
                       if (w!=null) {
                           if (debug&&line[titlefieldnames[9+j]] == "✅") { do_debug("Status adjusted as a record exists for NEW  "+w.Role+"/"+w['Unit name']+"/"+w['Membership number']+"/"+w.Status);}
                           line[titlefieldnames[9+j]] = "❌"
                           state = 0x274C;
                       }
                       w = welcome.data.find(e=>e['Membership number']==role_detail.data[i].membershipid&&e['Role']==role_detail.data[i].Role&&e['Team ID']==role_detail.data[i].TeamId&& e.CategoryKey == act[j]&&e['Unit name']==role_detail.data[i].UnitName&&e.Status=="Outstanding")
                       if (w!=null) {
                            if (debug&&line[titlefieldnames[9+j]] == "✅") { do_debug("Status adjusted as a record exists for Outstanding  "+w.Role+"/"+w['Unit name']+"/"+w['Membership number']+"/"+w.Status);}
                           line[titlefieldnames[9+j]] = "❌"
                           state = 0x274C;
                       }
                   }
                   if (role_detail.data[i].RoleStatusName=="Full") {
                       if (debug&&line[titlefieldnames[9+j]] != "✅") { do_debug("Status adjusted to Complete as Role is complete  "+w.Role+"/"+w['Unit name']+"/"+w['Membership number']);}
                       // DE sometimes seems to send us two rows with a different status - DE assumes if the role is full then status doesn't matter so we will do the same
                       line[titlefieldnames[9+j]] = "✅";
                       state = 0x2705;
    ...
    function report_onclick2() {
        ...
        keepalive = window.setInterval(function () {
            ...
            console.log("Keep Alive Active");
              var tokexpDate = new Date( JSON.parse(localStorage.getItem("authInfo")).account.idTokenClaims.exp *1000);
            console.log("Token expires :"+ tokexpDate.toGMTString());
...
table.tabulator-print-table, tr, th, td {    border: solid;font-family: sans-serif;}</style>';
   ...
   //     mems = GM_getValue("orgmems", []);
    ...
    function export_organogram() {
        if (!mems || mems.length === 0) {
            $("#gmorgstatus")[0].innerHTML = "No data to export — please run Chart Data first.";
            return;
        }
        var json = JSON.stringify(mems, null, 2);
        var blob = new Blob([json], { type: "application/json" });
        var url = window.URL.createObjectURL(blob);
        var a = document.createElement("a");
        a.style = "display: none";
        a.href = url;
        a.download = "organogram_data.json";
        document.body.appendChild(a);
        requestAnimationFrame(() => {
            a.click();
            window.URL.revokeObjectURL(url);
        ...
        $("#gmorgstatus")[0].innerHTML = "Exported " + mems.length + " nodes to organogram_data.json";
    }
...
function save_dashboardHTML() {
  GM_setValue("dashboardHTML", document.getElementById("dashboardtext").value)
}
    function del_dashboardHTML() {
  GM_deleteValue("dashboardHTML")
}
...
function report_download() {
    ...
    function open_dashboardHTML() {
   var dashWindow = window.open();
   report_download();
   var dbhtml = document.getElementById("dashboardtext").value;
   var finds = '<div id="pasteInput" contenteditable="true">';
   var findp =     dbhtml.indexOf(finds);
   dbhtml = dbhtml.slice(0, findp+finds.length)+   tabletabulator.getHtml() + dbhtml.slice(findp+finds.length);
   dashWindow.document.write(dbhtml);
}
    var permit_down = [];

    function get_permits(pn,permit_type) {
  const fields = ["Id", "PermitId", "PermitCategory", "FullName", "PermitType", "unitName", "County", "unitid","ExpiryDate","PermitActivity","MembershipNumber"];
           const requestBody = {
               contactId: "cedd2041-7e66-5133-2082-21802dcab3d0",
               id: "",
               name: "",
               query: "PermitActivityId = '"+permit_type+"'",
               distinct: true,
               isDashboardQuery: false,
               table: "PermitsView",
               selectFields: fields,
          ...
          GM_xmlhttpRequest({
                    ...
                    onload: function(response) {
                       console.log(JSON.parse(response.responseText));
                        var memrec = JSON.parse(response.responseText);
                        permit_down=permit_down.concat(memrec.data);
                        console.log(permit_down);
                        if(memrec.nextPage!="") {
                        get_permits(memrec.nextPage,permit_type)
                        } else {console.log(permit_down);
                        const worksheet = XLSX.utils.json_to_sheet(permit_down);
...
document.body.style.cursor = "default";
    ...
    function down_permit() {
       if (document.URL.includes("https://membership.scouts.org.uk/#/programmesupport/permits/permitsearch/")) {
           var permit_type = document.URL.split('/');
           console.log(permit_type);
            permit_down = [];
           document.body.style.cursor = "wait";
           alert("Click OK to Prepare Data - a filesave popup will appear when this is complete");
           get_permits(1,permit_type[7]);


    } else {alert("You need to be on a permit search screen for this to work");
};
 }
    function data_explore_navigate() {
var element = document.querySelector('[data-field="Membership number"][tabindex="0"]');

                if (element != null) {
      //  console.log(element);
      //  console.log(element.children[0].textContent);
        //{"pagesize":10,"nexttoken":1,"filter":{"global":"","globaland":false,"fieldand":true,"filterfields":[{"field":"membershipnumber","value":"0000107751"}]},"isSuspended":false}
             ...
             GM_xmlhttpRequest({
                    ...
                    context: 1,
                    url: "https://tsa-memportal-prod-fun01.azurewebsites.net/api/MemberListingAsync",
                    data: '{"pagesize":10,"nexttoken":1,"filter":{"global":"","globaland":false,"fieldand":true,"filterfields":[{"field":"membershipnumber","value":"'+element.children[0].textContent+'"}]},"isSuspended":false}',
                    //                  synchronous:    true,
                    headers: {
                        "Content-Type": "application/json",
                        "Authorization": "Bearer " + JSON.parse(localStorage.getItem("authInfo")).idToken,
                    ...
                    onload: function(response) {
                        console.log(JSON.parse(response.responseText));
                        var memrec = JSON.parse(response.responseText);
                        if (memrec.data.length>0) {
                            var urltogo = "https://membership.scouts.org.uk/#/membersearch/"+memrec.data[0].id+"/viewmember";
                            window.open(urltogo);
                        }
                        if (debug) { do_debug("Report 3: Roles Blob request for "+response.context+" ")}
                    },
    ...
    function show_assessor() {
         GM_xmlhttpRequest({
                    ...
                    url: "https://tsa-memportal-prod-fun01.azurewebsites.net/api/GetRelatedContactAsync",
                  data: '{"queryJSON":{"pageSize":10,"filter":{"fieldand":true,"global":"","globaland":false,"filterfields":[{"field":"fullName","value":"%Atkinson%"}]},"sort":{"field":"","type":""}},"pageSize":10,"filter":{"fieldand":true,"global":"","globaland":false,"filterfields":[{"field":"fullName","value":"%Atkinson%"}]},"sort":{"field":"FullName","type":"desc"},"permitActivityId":"6463594f-3626-ee11-9965-0022484257f7","permitCategoryId":"ba473763-3626-ee11-9965-0022484257f7","contactId":"cedd2041-7e66-5133-2082-21802dcab3d0","nexttoken":"1"}',
             headers: {
                        "Content-Type": "application/json",
                        "Authorization": "Bearer " + JSON.parse(localStorage.getItem("authInfo")).idToken,
                        "Accept": "application/json, text/plain, */*"
...
<input   type="radio" id="gmrole" name="gmreporttype" checked /> Role Report  \
<input   type="radio" id="gmvac" name="gmreporttype"  /> Vacancy Report  \
...
<span id="gmdashxx"><button id="gmopendash">Open Dashboard</button></span> \
...
<div id="dashboard" style=""><button id="gmsavedash">Save Dashboard</button><button id="gmdeldash">Delete Dashboard</button><br><textarea id="dashboardtext"></textarea></div> \
</div> ';
    //<p><input   type="checkbox" id="gmexplorer" name="gmexplorer" /> Experimental Data Collection</p> \
// Add Color Selectors
document.body.appendChild (newHTML);
if(GM_getValue("dashboardHTML")!=undefined) {
 document.getElementById("dashboardtext").value = GM_getValue("dashboardHTML");
}
document.getElementById("gmsavedash").onclick = save_dashboardHTML;
    document.getElementById("gmdeldash").onclick = del_dashboardHTML;
    document.getElementById("gmopendash").onclick = open_dashboardHTML;
...
<button id="gmRunOrg">Chart Data</button><button id="gmSaveOrg">Save</button> <button id="gmDownOrg">Download</button> <button id="gmExportOrg">Export JSON</button>\
...
//GM_setValue("orgmems", ""); // Clear any stored data following removal of org/map function
GM_deleteValue("orgmems");
// Row 1 (top 30px) - spread evenly left to right
addButton("Organogram",            show_organogram, {position:'absolute', top:'30px', left:'20%', 'z-index':3}, "");
addButton("Organogram (Assessor)", show_assessor,   {position:'absolute', top:'30px', left:'30%', 'z-index':3}, "");
addButton("Permit Holders",        down_permit,     {position:'absolute', top:'30px', left:'43%', 'z-index':3}, "");
addButton("Map",                   show_map,        {position:'absolute', top:'30px', left:'55%', 'z-index':3}, "");
addButton("Map Groups",            run_map,         {position:'absolute', top:'30px', left:'60%', 'z-index':3}, "");
// Row 2 (top 63px)
addButton("Clear Cookies",         clear_cookies,   {position:'absolute', top:'63px', left:'20%', 'z-index':3}, "");
addButton("Role Report",           report_onclick2, {position:'absolute', top:'63px', left:'30%', 'z-index':3}, "");
...
document.getElementById("gmExportOrg").onclick = export_organogram;
...
// Key press for Data Explorer navigation to Member
document.addEventListener('keypress', function(event) {
    if ((event.ctrlKey==true)&&(event.shiftKey==true)&&(event.charCode==11)) {
        data_explore_navigate();
    }
    ...
    for (let mutation of mutationsList) {
        if ((mutation.target.baseURI.includes("/dataexplorer/"))&&(mutation.target.baseURI.includes("/viewquery"))) {
        if ((mutation.type === 'attributes')&&(mutation.attributeName=="tabindex")&&(mutation.target.tabIndex==0)&&(mutation.target.dataset.field=="Membership number")) {
        //    console.log('A child node has been added or removed.');
    //    console.log(mutation);
      //      console.log(mutation.target.innerText)
            document.querySelectorAll('#nav_to_member').forEach(el => el.remove());

            const changedElement = mutation.target;
            const badge = document.createElement('span');
     // badge.textContent = ' (Updated!)';
                  badge.innerHTML = '<svg xmlns="http://w3.org" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"></path><polyline points="15 3 21 3 21 9"></polyline><line x1="10" y1="14" x2="21" y2="3"></line></svg>';
      badge.id = "nav_to_member";
            badge.style.cursor = 'pointer';
      badge.style.fontSize = '10px';
 badge.onclick = (e) => {
      e.stopPropagation();

  // 1. Change the pointer to a timer immediately
  badge.style.cursor = 'wait';
    ...
    badge.style.cursor = 'pointer';
      ...
      // Add it directly to the mutated element
      changedElement.appendChild(badge);
        }
     }
    }
      //  console.log("Chnage");
